    ...
    setFlows: function(opts) {
        return new Promise(function(resolve,reject) {
            ...
            apiPromise.then(function(flowId) {
                if ((deploymentType !== "reload") &&
                    flows.hasOwnProperty("credentials")) {
                    var cred = flows.credentials;
                    var secret = undefined;
                    if (flows.hasOwnProperty("credentialSecret")) {
                        secret = flows.credentialSecret;
                    }
                    var credPromise = runtime.nodes.loadCredentials(cred, secret);
                    credPromise.then(function () {
                        return resolve({rev:flowId});
                    }).catch(function (err) {
                        runtime.log.warn(runtime.log._("api.flows.credentials"),{message:err.message});
