...
RED.editor = (function() {
    ...
    function buildInfoForm(container, node, subflowEditor) {
        ...
        var exportName = node.exportName ? node.exportName : "";
        name.val(exportName);
        ...
        var licenses = ["Apache-2.0", "BSD-3-Clause", "BSD-2-Clause", "GPL-2.0", "GPL-3.0", "MIT", "MPL-2.0", "CDDL-1.0", "EPL-2.0", "none"];
        licenseSelector.append($("<option/>").val("_select_").text(RED._("subflow.selectLicense")));
        ...
        var lic = node.license;
        license.val(lic ? lic : "");
        if (licenses.includes(lic) || (lic === "_custom_")) {
            licenseSelector.val(lic);
        }
        else {
            licenseSelector.val("_select_");
        }
        ...
        var version = $("<input/>", {
            id: "subflow-info-input-version",
            type: "text",
            "data-i18n": "[placeholder]editor:subflow.placeholder.version"
        }).appendTo(versionRow);
        ...
        $("<label/>", {
            for: "subflow-info-input-sealed",
            "data-i18n": "editor:subflow.sealed"
        }).css({
            width: "auto",
            "margin-left": "10px"
        }).appendTo(sealedRow);
        sealed.prop("checked", (node.sealed === true));
        ...
        var includeAll = $("<input/>", {
            type: "checkbox",
            id: "subflow-info-input-includeAll"
        }).css({
            width: "auto",
            "vertical-align": "top",
            "margin-left": "100px"
        ...
        $("<label/>", {
            for: "subflow-info-input-includeAll",
            "data-i18n": "editor:subflow.includeAll"
        }).css({
            width: "auto",
            "margin-left": "10px"
        }).appendTo(incRow);
        if (!node.hasOwnProperty("includeAll")) {
            node.includeAll = true;
        }
        includeAll.prop("checked", (node.includeAll === true));
    }

    function isEditableSubflow(node) {
        var sf = RED.nodes.subflow(node.type.substring(8));
    ...
    function showEditDialog(node) {
        ...
        console.log("; sED", type, editing_node);
        if ((type === 'subflow') &&
            isEditableSubflow(editing_node)) {
    ...
    function commitSubflowEdit(editing_node, subflowEditor, done) {
        ...
        new_exportName = new_exportName.replace(/[^A-Za-z0-9_\-]/g, '_');
        ...
        var new_version = $("#subflow-info-input-version").val();
        ...
        if (JSON.stringify(old_keywords) !== JSON.stringify(new_keywords)) {
        ...
        var old_sealed  = editing_node.sealed;
        var new_sealed = $("#subflow-info-input-sealed").prop("checked");
        if (old_sealed !== new_sealed) {
            editing_node.sealed = new_sealed;
            changes.sealed = new_sealed;
            changed = true;
        }

        var old_inc  = editing_node.includeAll;
        var new_inc = $("#subflow-info-input-includeAll").prop("checked");
        if (old_inc !== new_inc) {
            editing_node.includeAll = new_inc;
            changes.includeAll = new_inc;
            changed = true;
        }
