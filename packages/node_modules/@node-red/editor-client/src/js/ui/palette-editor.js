...
RED.palette.editor = (function() {
    ...
    function refreshNodeModuleList() {
        for (var id in nodeEntries) {
            if (nodeEntries.hasOwnProperty(id)) {
                ...
                else if (entry.type === "subflow") {
                    refreshSubflowNode(entry.info);
                }
            }
        }
    }
    ...
    function refreshSubflowNode(sf) {
        var name = sf.type;
        if (!nodeEntries.hasOwnProperty(name)) {
            nodeEntries[name] = {info:sf, type: "subflow"};
            nodeList.editableList("addItem", nodeEntries[name]);
        }
        else {
            console.log("; same subflow?", sf);
        }
    }
    ...
    function removeSubflow(sf) {
        ...
        for (var i = 0; i < removedNodes.length; i++) {
            RED.nodes.remove(removedNodes[i].id);
        }
        
        RED.nodes.removeSubflow(name);
        if (nodeEntries.hasOwnProperty(id)) {
            console.log("; rSF/1:", id);
            nodeList.editableList("removeItem", nodeEntries[id]);
            delete nodeEntries[id];
        }
    }
    ...
    function _refreshNodeModule(module) {
        ...
        if (!nodeEntries.hasOwnProperty(module)) {
            nodeEntries[module] = {info:RED.nodes.registry.getModule(module), type: "module"};
            var index = [module];
            ...
            var nodeType = nodeEntries[module].type;
            if (nodeEntry && (nodeType == "module")) {
                ...
                for (var setName in moduleInfo.sets) {
                    if (moduleInfo.sets.hasOwnProperty(setName)) {
                        ...
                        console.log("; sE", setElements, setName);
                        if (set.err) {
                            errorCount++;
    ...
    function init() {
        ...
        RED.events.on('registry:node-type-added', function(nodeType) {
            if (!/^subflow:/.test(nodeType)) {
                ...
                console.log("; rnta:", nodeType, sf);
                if (!sf.editable) {
                    refreshSubflowNode(sf);
                }
            }
        });
        RED.events.on('registry:node-type-removed', function(nodeType) {
            if (!/^subflow:/.test(nodeType)) {
                ...
                console.log("; rntr:", nodeType);
                removeSubflow(nodeType);
            }
        });
    ...
    function createSettingsPane() {
        ...
        nodeList = $('<ol>',{id:"red-ui-palette-module-list", style:"position: absolute;top: 35px;bottom: 0;left: 0;right: 0px;"}).appendTo(modulesTab).editableList({
            ...
            sort: function(A,B) {
                ...
                var nameA = entryToName(A);
                var nameB = entryToName(B);
                console.log("; A:", nameA, A, "B:", nameB, B);
            ...
            addItem: function(container,i,object) {
                ...
                console.log("; aI", object, type);
                if (entry) {
                    var headerRow = $('<div>',{class:"red-ui-palette-module-header"}).appendTo(container);
                    var icon = (type == "module") ? "fa-cube" : "fa-object-group ";
                    var titleRow = $('<div class="red-ui-palette-module-meta red-ui-palette-module-name"><i class="fa '+icon+'"></i></div>').appendTo(headerRow);
                    var name = (type == "module") ? entry.name : entry.exportName;
                    $('<span>').text(name).appendTo(titleRow);
                    ...
                    var setCount = null;
                    if (type === "module") {
                        setButton = $('<a href="#" class="red-ui-button red-ui-button-small red-ui-palette-module-set-button"><i class="fa fa-angle-right red-ui-palette-module-node-chevron"></i> </a>').appendTo(buttonRow);
                        setCount = $('<span>').appendTo(setButton);
                    }
                    var buttonGroup = $('<div>',{class:"red-ui-palette-module-button-group"}).appendTo(buttonRow);

                    var updateButton = null;
                    if (type === "module") {
                        updateButton = $('<a href="#" class="red-ui-button red-ui-button-small"></a>').text(RED._('palette.editor.update')).appendTo(buttonGroup);
                    ...
                    removeButton.on("click", function(evt) {
                        evt.preventDefault();
                        remove(entry,type,container,function(err){});
                    })
                    if ((type === "module") && !entry.local) {
                        removeButton.hide();
                    }
                    var enableButton = null;
                    if (type === "module") {
                        enableButton = $('<a href="#" class="red-ui-button red-ui-button-small"></a>').text(RED._('palette.editor.disableall')).appendTo(buttonGroup);
                    }
    ...
    function remove(entry,type,container,done) {
        if (RED.settings.theme('palette.editable') === false) {
            done(new Error('Palette not editable'));
            return;
        }
        var name = (type === "module") ? entry.name : entry.type;
        var notification = RED.notify(RED._("palette.editor.confirm.remove.body",{module:name}),{
            modal: true,
            fixed: true,
            buttons: [
                {
                ...
                },
                {
                    ...
                    click: function() {
                        ...
                        if (type == "module") {
                            ...
                            RED.eventLog.startEvent(RED._("palette.editor.confirm.button.remove")+" : "+name);
