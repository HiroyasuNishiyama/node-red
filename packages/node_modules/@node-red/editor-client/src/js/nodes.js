...
RED.nodes = (function() {
    ...
    function addSubflow(sf, createNewIds) {
        ...
        RED.nodes.registerType("subflow:"+sf.id, {
            ...
            incomplete: function() {
                var ic = sf.incomplete;
                var ret = (ic && (ic.length > 0)) ? ic : [];
                return ret;
            },
            version: sf.version,
    ...
    function refreshIncompleteSealedSubflow(sf) {
        if (sf.incomplete) {
            return sf.incomplete;
        }
        if (isEditableSubflow(sf)) {
            sf.incomplete = [];
            return sf.incomplete;
        }
        var incomplete = new Set();
        var nodes = RED.nodes.filterNodes({z:sf.id});
        nodes.forEach(function (n) {
            var type = n.type;
            if (type === "unknown") {
                incomplete.add(n.name);
            }
            else if (type.substring(0, 8) == "subflow:") {
                var subflowId = type.substring(8);
                var sf1 = getSubflow(subflowId);
                if (sf1) {
                    var incomplete1 = sf1.incomplete;
                    if (!incomplete1) {
                        incomplete1 = refreshIncompleteSealedSubflow(sf1);
                    }
                    incomplete1.forEach(function (e) {
                        incomplete.add(e);
                    });
                }
                else {
                    incomplete.add(n.type);
                }
            }
        });
        sf.incomplete = Array.from(incomplete);
        return sf.incomplete;
    }

    function refreshSealedSubflow() {
        var subflows = [];
        RED.nodes.eachSubflow(function (sf) {
            if (!isEditableSubflow(sf)) {
                sf.incomplete = null;
        ...
        subflows.forEach(function (sf) {
            if (!sf.incomplete) {
                refreshIncompleteSealedSubflow(sf);
            }
        });
    }
