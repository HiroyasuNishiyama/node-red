...
RED.nodes = (function() {
    ...
    function isEditableSubflow(sf) {
        if (sf.sealed && sf.imported) {
            return false;
        }
        return true;
    }
    ...
    function addSubflow(sf, createNewIds) {
        ...
        RED.nodes.registerType("subflow:"+sf.id, {
            ...
            editable: isEditableSubflow(sf),
            incomplete: function() { return sf.incomplete; },
            version: sf.version,
            exportName: sf.exportName
        });
    ...
    function collectNodesInSubflow(n, isSealed, isAll) {
        var nodes = RED.nodes.filterNodes({z:n.id});
        return createExportableNodeSet(nodes, {}, {}, isSealed, isAll);
    }
    
    function convertSubflow(n, isSealed, isAll) {
        console.log("; cS:", n, isSealed, isAll);
        var node = {};
        var id = n.id;
        if (n.flow) {
            var sealed = isSealed || n.sealed
            var all = isAll || n.includeAll;
            var flow = collectNodesInSubflow(n, sealed, all);
            node.flow = flow;
        }
        else if (n.flow === null) {
            node.flow = null;
        ...
        node.sealed = n.sealed;
        node.includeAll = n.includeAll;
    ...
    function isExportableSubflow(sf) {
        if (sf.hasOwnProperty("imported") && !sf.imported) {
            return false;
        }
        if (sf.hasOwnProperty("exportName") &&
            (sf.exportName !== undefined) &&
            (sf.exportName !== "")) {
            return false;
        }
        return true;
     ...
     **/
    function createExportableNodeSet(set, exportedSubflows, exportedConfigNodes, isSealed, isAll) {
        var nns = [];
        ...
        for (var n=0;n<set.length;n++) {
            ...
            if (node.type.substring(0,8) == "subflow:") {
                ...
                if (!exportedSubflows[subflowId]) {
                    ...
                    console.log("; S:", node, subflow, [isSealed, isAll]);
                    if (!isSealed || isAll || isExportableSubflow(subflow)) {
                        ...
                        var sealed = isSealed || subflow.sealed;
                        var all = isAll || subflow.includeAll;
                        var exportableSubflow = createExportableNodeSet(subflowSet, exportedSubflows, exportedConfigNodes, sealed, all);
                        nns = exportableSubflow.concat(nns);
                    }
                    else if (subflow.hasOwnProperty("exportName") &&
                             (subflow.exportName !== "")) {
                        var newNode = cloneObject(node);
                        newNode.type = "subflow:"+subflow.exportName;
                        node = newNode;
                        set[n] = newNode;
                    }
                }
    ...
    function replaceNodeID(node, nodes, old_id, new_id) {
        console.log("; RNID:", old_id, new_id);
        if (node.id == old_id) {
            node.id = new_id;
        }
        if (node.z == old_id) {
        ...
        if (node.type == "subflow:"+old_id) {
            node.type = "subflow:"+new_id;
        }
        ["in", "out"].forEach(function (inOut) {
            if (node.hasOwnProperty(inOut) &&
                node[inOut]) {
                var ports = node[inOut];
                if (ports) {
                    ports.forEach(function (port) {
                        var wires = port.wires;
        ...
        if (flow) {
            flow.forEach(function (n) {
                replaceNodeID(n, null, old_id, new_id);
            });
        }
        if (nodes) {
    ...
    function replaceSubflowID(sf, nodes) {
        if (sf.exportName && (sf.exportName !== "")) {
            var old_id = sf.id;
            //var new_id = "subflow:"+sf.exportName;
            var new_id = sf.exportName;
            replaceNodeID(sf, nodes, old_id, new_id);
        }
    }

    function markIncompleteSubflow(id, subflow_map) {
        var sf = subflow_map[id];
        if (sf && !sf.incomplete) {
            console.log("; IC:", sf);
            sf.incomplete = true;
            markIncompleteSubflow(sf.z, subflow_map);
        }
    }
    
    function importNodes(newNodesObj,createNewIds,createMissingWorkspace) {
        console.log("; iN:", createNewIds, createMissingWorkspace);
        var i;
        var n;
        var newNodes;
        var nodeZmap = {};
        ...
        newNodes.forEach(function (n) {
            ...
            if ((n.type === "subflow") && n.hasOwnProperty("flow")) {
                ...
                if (n.sealed) {
                    if (n.includeAll) {
                        flow.forEach(function (nn) {
                            if (nn.type === "subflow") {
                                nn.hidden = true;
                                console.log("; Hidden", nn);
                            }
                        });
                    }
                }
                if (flow) {
                    flow.forEach(function(nn) {
                        if (nn.type === "subflow") {
                            replaceSubflowID(nn, flow);
                            nn.flow = null;
                            nn.imported = true;
                        }
                    });
                }
                replaceSubflowID(n, flow);
                n.flow = null;
                n.imported = true;
            }
        });
        newNodes = tmp;
        console.log("; NN:", cloneObject(newNodes, true));
        ...
        for (i=0;i<newNodes.length;i++) {
            ...
            if (n.type === "workspace" || n.type === "tab") {
                ...
                console.log("; SF:", n.id, n, matchingSubflow);
                if (matchingSubflow) {
                    subflow_blacklist[n.id] = matchingSubflow;
                } else {
                    subflow_map[n.id] = n;
                    var createID = createNewIds && !n.imported;
                    if (createID) {
                        nid = getID();
                        n.id = nid;
                    }
                    console.log("; ID:", n.id, createNewIds);
        ...
        for (i=0;i<newNodes.length;i++) {
            ...
            if (n.type !== "workspace" && n.type !== "tab" && n.type !== "subflow") {
                ...
                if (!def || def.category != "config") {
                    ...
                    if (createNewIds) {
                        console.log("; sm/", node.z, subflow_map[node.z]);
                        if (subflow_blacklist[n.z]) {
                            continue;
                    ...
                    if (n.type.substring(0,7) === "subflow") {
                        ...
                        if (subflow) {
                            ...
                            node.name = type;
                            node.type = "unknown";
                            markIncompleteSubflow(node.z, subflow_map);
                            RED.notify("<p>Unknown subflow: "+ type+"</p>","error",false,10000);
                        }
                    } else {
                        if (!node._def) {
                            if (node.x && node.y) {
