...
RED.nodes = (function() {
    ...
    function isExportableSubflow(sf) {
        return (sf && sf.exportName && (sf.exportName !== ""));
    }
    
    function collectNodesInSubflow(n, isSealed) {
        var id = n.id;
        var nodes = RED.nodes.filterNodes({z:id});
        var set = createExportableNodeSet(nodes);
        var newSet = [];
        set.forEach(function (nn) {
            if (nn.type === "subflow") {
                nn.exportName = null;
            }
            newSet.push(nn);
    ...
    function convertSubflow(n, isSealed) {
        var node = {};
        node.id = n.id;
        node.type = n.type;
        node.name = n.name;
        ...
        if (n.flow) {
            ...
            var flow = collectNodesInSubflow(n, sealed);
            node.flow = flow;
        }
        else if (n.flow === null) {
            node.flow = null;
